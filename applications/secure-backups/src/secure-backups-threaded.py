...
def process_object(task: dict, c_bucket, db_secrets_vals, multipart_upload_block: dict, upload_id: str, ):
    ...
    copy_source = {'Bucket': task["source_bucket"],
                   'Key': task["source_key"]}
    response = c_bucket.upload_part_copy(Bucket=task['target_bucket'],
                                         CopySource=copy_source,
                                         CopySourceRange=task['byte_range'],
                                         Key=task['target_obj'],
                                         PartNumber=str(task['part']),
                                         UploadId=upload_id)
    part_block = {'ETag': response['CopyPartResult']['ETag'],
                  'PartNumber': response['CopyPartResult']['PartNumber']}
    if response['CopyPartResult']['ChecksumCRC32']:
        part_block['ChecksumCRC32'] = response['CopyPartResult']['ChecksumCRC32']
    if response['CopyPartResult']['ChecksumCRC32C']:
        part_block['ChecksumCRC32C'] = response['CopyPartResult']['ChecksumCRC32C']
    if response['CopyPartResult']['ChecksumSHA1']:
        part_block['ChecksumSHA1'] = response['CopyPartResult']['ChecksumSHA1']
    if response['CopyPartResult']['ChecksumSHA256']:
        part_block['ChecksumSHA256'] = response['CopyPartResult']['ChecksumSHA256']
    multipart_upload_block['Parts'].append(part_block)
            ...
            if task_list:
                ...
                for task in task_list:
                    ...
                    for entry in upload_map:
                        ...
                        thread_list.append({'file_id': task['file_id'], 'obj_id': task['obj_id'],
                    ...
                    mpu = s3_client.create_multipart_upload(task['target_bucket'],
                                                            task['target_obj'])
                    while True:
                        thread_len = len(thread_list)
                        if thread_len == 0:
                            break
                        elif thread_len > max_th:
                            thread_range = max_th
                        else:
                        ...
                        for thread in threads:
                            thread.start()
                            thread.join()
                        thread_list = thread_list[thread_range:]
                    s3_client.complete_multipart_upload(task['target_bucket'], task['target_obj'],
                                                        multipart_upload_block, mpu)
